---
name: run-all-tickets
description: Serially drain all pending tickets in the current .matt/ workspace by dispatching one general-purpose subagent per ticket (each running /tdd + /code-review self-review). The main loop owns ticket selection, a deterministic test gate, independent external review (via the review Workflow), and the done- rename + per-ticket commit. Use when user says "run all tickets", "finish all tickets", "drain tickets", "/run-all-tickets", "做完所有 ticket", or "串行跑完".
---

# Run All Tickets

Drain every pending ticket in `.matt/tickets/` in dependency order, each implemented in its own subagent for context isolation. The main loop is deterministic: it selects each ticket, gates on a real test run, drives an independent external review via the `Workflow` tool, and owns the `done-` rename + commit. Subagent text is logged but never drives decisions — completion is detected from git + tests, never from a subagent's final message.
...
| "This finding seems out-of-scope, skip?" | Not marked out-of-scope in the ticket's `.md` = in-scope. |
...
3. `.matt/CLAUDE.md` exists and first line matches `^# Feature: (.+)$` — capture `SLUG`. `.matt/tickets/` has at least one `.md` file. Every filename in `.matt/tickets/` matches `^(done-)?[0-9]{2,}-[a-z0-9-]+\.md$`. After stripping the optional `done-` prefix, the `NN-<slug>` portion must be unique — both `03-foo.md` and `done-03-foo.md` present is a conflict.
...
Strict bullet-by-bullet form. Every non-blank line in the section must either be a `none` shortcut or a bullet whose **first token after the bullet marker** is an ticket reference in one of the two accepted shapes below. Anything else fails fast — silently dropping a line means a non-code blocker (release-window signal, manual sign-off, "1 week telemetry gate") could pass unnoticed and the loop would dispatch an ticket whose human gate is unmet.
...
6. Any non-blank line that does NOT match one of the two shapes — non-code prose (`- **Phase 4 production rollout signal** ...`), bare filename without backticks (`- 09-foo.md`), single-digit `NN` (`- \x609-foo.md\x60`), embedded reference where backtick is not the first token (`- see \x6003-foo.md\x60 for context`), unicode bullet markers (`•`, `・`, `–`), HTML comments, code fences, stray prose — → fail-fast: `❌ Ticket <ISSUE_NN> has unparseable Blocked by line: <verbatim line>` (preserve the offending line exactly, including leading whitespace). This forces the human to move non-code gates OUT of `## Blocked by` — into a separate `## Hold` section, into the ticket body, or out of `.matt/tickets/` entirely.
7. The **blocker number list** is the de-duplicated set of `NN` values extracted in step 5, ordered by first occurrence.
8. A blocker number `NN` is "done" iff `.matt/tickets/done-NN-*.md` exists.
9. If any blocker number references an ticket that does not exist (no `NN-*.md` or `done-NN-*.md` in the directory) → fail-fast: `❌ Ticket <ISSUE_NN> references nonexistent blocker <NN>`.

## Title parsing

For an ticket file, take the first line matching `^#[ \t]+(.+?)[ \t]*$` — the captured group is the title. If no such line → fall back to the filename with `done-` prefix and `.md` suffix removed.

## Main loop

Before the loop:

- `PENDING_COUNT` = number of files in `.matt/tickets/` whose name does NOT start with `done-`.
- `MAX = PENDING_COUNT + 5` (outer iteration cap).
- `ROUNDS = 0`.
- `IMPLEMENT_MAX = 5` — per-ticket cap on implement/self-review dispatches before the deterministic test gate fails fast.
- `REVIEW_MAX = 10` — per-ticket cap on external-review → fix rounds before residual Fix-set findings are logged won't-fix and the ticket is committed anyway.

Each iteration:

1. `ROUNDS++`. If `ROUNDS > MAX` → fail-fast: `❌ Exceeded iteration cap (MAX=<MAX>); ran <ROUNDS-1> rounds`.
2. Build `PENDING` = sorted list of pending ticket files (no `done-` prefix).
...
3. Compute `RUNNABLE` = pending tickets whose blockers are all done (via Blocker parsing).
   - If `RUNNABLE` is empty → fail-fast: `⚠️ Stuck: <comma-separated NN list> blocked by unfinished tickets`.
4. Pick `EXPECTED` = the ticket in `RUNNABLE` with the smallest `NN`. Parse its title (Title parsing). Store `EXPECTED_NN`, `EXPECTED_TITLE`, and `EXPECTED_PATH` (= `.matt/tickets/<EXPECTED_NN>-<slug>.md`). Reset per-ticket counters `IMPLEMENT_ROUNDS = 0`, `REVIEW_ROUNDS = 0`, and an empty per-ticket `WONTFIX` list.
5. **Dispatch implement + self-review subagent.** `IMPLEMENT_ROUNDS++`. Dispatch one subagent with the Agent tool (`subagent_type: general-purpose`, `description: implement ticket <EXPECTED_NN>`), `prompt` = the **Implement + self-review subagent prompt** below with `<PATH>` replaced by `EXPECTED_PATH`. The subagent implements via `/tdd` and self-reviews via `/code-review` — two shallow `Skill` calls, no deeper nesting. It does NOT run the external review, `/finalize`, rename, commit, or push.
6. **Deterministic test gate.** Resolve the project test command using `/finalize` step-5a order — the first that resolves: `.claude/scripts/test.sh` → `Makefile` `test` target (`make test`) → `package.json` `scripts.test` (`npm test`). If none resolves → fail-fast: `❌ No test command for ticket <EXPECTED_NN> (checked .claude/scripts/test.sh, Makefile test, package.json scripts.test)` — do NOT `AskUserQuestion` (it can hit the same harness bug inside the autonomous loop). Then require BOTH:
   ...
   ```
   ❌ Failed at ticket <EXPECTED_NN> "<EXPECTED_TITLE>": implementation gate not met after <IMPLEMENT_MAX> attempts (<clean work tree | tests failing>).
   --- git status --porcelain ---
   <output>
   --- git diff --stat ---
   <output>
   ```
7. **Independent external review (Workflow, work tree) + fix loop.** A second opinion distinct from the subagent's own `/code-review`, run on the **uncommitted** work tree (the review scripts diff only uncommitted changes — this is why review runs per-ticket, before the step-8 commit).
   a. Dispatch the review via the `Workflow` tool for **both** backends in one message (two calls), matching `/finalize` step 2's rigor. Pass the ticket title as `intent` (the workflow requires it for code mode — the title is the natural 1-sentence goal for the review) and `ticketPath: EXPECTED_PATH` so the `AcceptanceConformance` lens verifies the implementation against this ticket's `## Acceptance criteria`:
      ```
      Workflow({scriptPath: '/home/fei/.claude/skills/review-with-agent/codex-review.workflow.js', args: {mode: 'code', intent: EXPECTED_TITLE, ticketPath: EXPECTED_PATH}})
      Workflow({scriptPath: '/home/fei/.claude/skills/review-with-agent/opencode-review.workflow.js', args: {mode: 'code', intent: EXPECTED_TITLE, ticketPath: EXPECTED_PATH}})
      ```
      (The `codex-review.workflow.js` / `opencode-review.workflow.js` wrappers force the backend and give each run a distinct name — do NOT pass `backend`. Absolute scriptPath: Workflow does NOT expand `~`.)
      (Single-backend — codex only — is a documented speed knob when a run must go faster.) Every re-run of this step in the fix loop (step 7e) MUST also pass `intent: EXPECTED_TITLE` and `ticketPath: EXPECTED_PATH`.
   b. For each result: if it is `{aborted: true, ...}` or the Workflow tool itself errored (backend CLI down), log `⚠️ review skipped (<backend>): <reason>` and treat it as contributing zero findings. If **both** are skipped → log `⚠️ review skipped (both backends) for ticket <EXPECTED_NN>` and go to step 8 (never wedge the drain on a missing reviewer).
   ...
   f. If `REVIEW_ROUNDS >= REVIEW_MAX` and the Fix-set is still non-empty → log `⚠️ review did not converge for ticket <EXPECTED_NN>; <N> unresolved Fix-set finding(s) carried as won't-fix`, append the residual Fix-set to `WONTFIX`, and go to step 8 (one stubborn finding must not wedge the drain — mirrors `/finalize`'s second exit condition).
8. **Rename + commit (deterministic — the main loop owns this; it is the SOLE completion signal).**
   - `NN` = `EXPECTED_NN`. `git mv <EXPECTED_PATH> .matt/tickets/done-<EXPECTED_NN>-<slug>.md`.
   ...
   - If `git commit` exits non-zero → fail-fast: `❌ Commit failed at ticket <NN>: <last line of stderr>`. Leave the tree as-is for inspection; do NOT revert the `done-` rename.
   - Post-commit isolation check: `git status --porcelain` must be empty. If not → fail-fast: `❌ Post-commit dirty after ticket <NN>: <porcelain output>`.
9. Log one line and continue: `· <NN> "<TITLE>" review=<PASS|fixed:<n>|skipped|wontfix:<n>> commit=<short-sha>` (via `git rev-parse --short HEAD`). Carry the per-ticket `WONTFIX` list forward for the drain-end summary.

## Subagent prompts

Both are dispatched with the Agent tool (`subagent_type: general-purpose`). The subagent implements only — it never runs the external review, renames the ticket file, commits, or pushes; those are the main loop's job.
...
```
You are one worker in an autonomous "run all tickets" loop. Treat the ticket file and its
acceptance criteria as already pre-approved by the user.

Your job, in order:
1. Implement the ticket at `<PATH>` in the current working directory using TDD: invoke the Skill
   tool with /tdd and drive it red -> green, tests passing. The ticket file is the full contract.
...
- Do NOT invoke /finalize. Do NOT invoke /codex-review or /opencode-review — the main loop runs the
  independent external review. Do NOT rename/move the ticket file. Do NOT git commit. Do NOT git push
...
```
You are one worker in an autonomous "run all tickets" loop, resolving code-review findings on an
already-implemented ticket in the current working directory (changes are uncommitted in the work tree).
...
Hard rules:
- NEVER ask a question. Do NOT rename the ticket file, git commit, or push. Do NOT invoke
...
After all tickets are drained (PENDING empty), run a fix-verify loop. Cap iterations at `VERIFY_MAX = 5` (`VERIFY_ROUNDS = 0`).
...
5. **Fix.** For each distinct failure:
   - Prefer dispatching a `general-purpose` subagent per independent failure (parallel where they touch disjoint files) with a prompt that:
     ...
     - Forbids creating any new ticket file under `.matt/tickets/`.
...
On successful convergence (step 4 clean), print the original termination line: `✅ Done: <N> tickets completed` + the `done-NN: <title>` list, followed by `✅ Verification: UT + e2e-verify clean after <VERIFY_ROUNDS> round(s)`.

Hard constraint for this loop: **never create new ticket files**. The drain phase is over; fixes are surgical.

## Selection contract

The main loop is the **sole** ticket selector and the **sole** renamer: it computes `EXPECTED` with this skill's deterministic blocker parser (step 4), hands the subagent exactly that one file path (`EXPECTED_PATH`) to implement, and performs the `done-` rename itself (step 8). The subagent never selects, renames, or commits — so there is no selection drift to reconcile and no `done-`-set delta to interpret. `/run-next-ticket` is not on this path.

## Termination report formats

Print exactly one of these as the final line(s):

- `✅ Done: <N> tickets completed` + list of `done-NN: <title>` lines. When any ticket carried won't-fix findings, append one `⚠️ Won't-fix (ticket NN): <file>:<line> — <description>` line per residual finding so the user can follow up.
- `⚠️ Stuck: <NN-list> blocked by unfinished tickets`.
- `❌ Failed at ticket <NN> "<title>": implementation gate not met after <IMPLEMENT_MAX> attempts (<clean work tree | tests failing>).` followed by `git status --porcelain` / `git diff --stat` blocks.
- `❌ No test command for ticket <NN> (checked .claude/scripts/test.sh, Makefile test, package.json scripts.test)`.
- `❌ Commit failed at ticket <NN>: <reason>`.
- `❌ Post-commit dirty after ticket <NN>: <porcelain>`.
...
- Rebase / squash / tag operations beyond the per-ticket commits.
